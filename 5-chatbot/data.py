...
def question_answers(id2line, convos):
    ...
    """ 生成 questions 和 answers 两个相同大小的数组"""
    questions, answers = [], []
    for convo in convos:
...
def token2id(data, mode):
    ...
    for line in lines:
        if mode == 'dec':  # we only care about '<s>' and </s> in encoder(? decoder?)
            ids = [vocab['<s>']]
        else:
            ids = []
...
def prepare_raw_data():
    ...
    id2line = get_lines() # 得到 L900: sentence 的字典
    convos = get_convos() # 得到 L900 这样的列表
    questions, answers = question_answers(id2line, convos) # 生成问答对，存在不同的数组
    prepare_dataset(questions, answers) # 分成训练和测试，写入到文件中
